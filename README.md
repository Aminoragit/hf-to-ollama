...
명령어 실행 직후 터미널에 나타나는 지름길과 선택지에 따라 단계적으로 쉽게 모델을 설치할 수 있습니다.
...
**진행 과정:**
1. Hugging Face 레포지토리 경로 입력 (예방식: `username/repo`)
...
단일 명령어로 필요한 모든 파라미터를 작성하여 사용자 프롬프트 개입 없이 곧바로 설치를 완료합니다. 파이프라인이나 스크립트 작성에 아주 유용하게 쓰입니다.
...
*💡 인증 토큰이 필요한 모델일 경우 명령어 맨 끝에 `--token <huggingface_access_token>` 옵션을 함께 적용하세요.*
...
위 과정에서 세팅한 값들을 기반으로, 백그라운드 환경에서 `ollama create` 에 전달되며 참조되는 `Modelfile`의 기본 구조는 아래와 같습니다:
